    ...
    >
      <div className="max-w-screen-xl mx-auto px-4 sm:px-8 py-3.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <img className="w-[max(8%,72px)]" src={assets.logo} alt="Logo" />
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={() => setToken("")}
            className="group flex items-center gap-2 bg-[#1a1a1a] text-white text-[11px] font-semibold tracking-[0.2em] uppercase rounded-full transition-colors duration-200
              /* Mobile: icon-only pill */
              p-2
              /* sm+: full pill with label */
              sm:px-5 sm:py-2.5"
          >
            <motion.span className="transition-transform duration-200 group-hover:-translate-x-0.5">
              {Icons.logout}
            </motion.span>
            {/* Label hidden on mobile to prevent crowding */}
