    ...
    <section className="relative w-full overflow-hidden">
      ...
      <div className="relative z-10 max-w-screen-xl mx-auto px-6 lg:px-12 xl:px-20 py-24">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-16">
          ...
          >
            ...
            <p className="mt-4 text-sm text-gray-400 leading-relaxed max-w-sm font-light">
              Handpicked silhouettes fresh off the runway — before they sell out.
            </p>
          </motion.div>
          ...
          >
            {Array(4).fill(null).map((_, i) => (
              <span key={i} className="flex items-center">
                {["New Arrivals", "Spring 2025", "Free Shipping", "50k+ Customers", "Ethically Made", "Members Get 20% Off"].map((t) => (
                  <span key={t} className="flex items-center gap-6 px-8">
                    <span className="text-[10px] font-bold tracking-[0.35em] uppercase text-[#414141]/40">{t}</span>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {(displayed.length ? displayed : latestProducts).map((item, idx) => (
              <motion.div
                key={item._id}
              ...
              >
                ...
                {idx < 3 && (
                  <motion.span
                    ...
                    transition={{ delay: idx * 0.1 + 0.4, type: "spring", stiffness: 400 }}
        ...
        <AnimatePresence>
          {displayed.length === 0 && activeFilter !== "All" && (
            ...
            >
              ...
              <p className="text-sm font-semibold text-gray-500">No {activeFilter} pieces yet</p>
              <button
        ...
        >
          ...
          <div className="flex items-center gap-4">
            <div className="flex -space-x-2.5">
              {["bg-violet-300", "bg-amber-300", "bg-emerald-300", "bg-pink-300"].map((c, i) => (
                <div key={i} className={`w-8 h-8 rounded-full ${c} border-2 border-white`} />
              ))}
            </div>
            <div>
              <p className="text-xs font-bold text-[#1a1a1a]">Loved by 50,000+</p>
