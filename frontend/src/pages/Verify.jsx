...
const Verify = () => {
  ...
  const verifyPayment = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        backendUrl + '/api/order/verifyStripe',  // ✅ Change from "verify-stripe" to "verifyStripe"
        {
          success,
          orderId,
          userId: userId
  ...
  useEffect(() => {
    console.log("🔄 Verify component mounted/updated with token:", token ? "Exists" : "Missing");
        ...
        {verifying ? (
          <>
            ...
            <div className="text-sm text-gray-500">
              ...
              <p>Status: {searchParams.get('success') === 'true' ? 'Success' : 'Pending'}</p>
            </div>
          </>
        ) : (
          <>
